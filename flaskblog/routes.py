...
@app.route('/comment/delete/<post_id>/<comment_id>')
@login_required
def del_comment(post_id, comment_id):
    post = Post.query.filter_by(id = post_id).first()
    comments = Comment.query.filter_by(post_id = post.id).order_by(Comment.posted.desc())
    comment = Comment.query.filter_by(id = comment_id).first()
    if post.user_id == current_user.id or comment.user_id == current_user.id:

        Comment.delete_comment(comment)

    return render_template('comments.html', post = post, comments = comments)

@app.route('/post/comment/new/<post_id>', methods = ['GET', 'POST'])
@login_required
